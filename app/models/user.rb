class User < ApplicationRecord
  ...
  has_many :comfirmed_friendships, -> { where confirmed: true }, class_name: 'Friendship', foreign_key: :inviter_id
  has_many :friends, through: :comfirmed_friendships, source: :invitee

  def friend_requests
    Friendship.where(invitee_id: self, confirmed: false)
  end

  def pending_requests
    Friendship.where(inviter_id: self, confirmed: false)
  end
end
